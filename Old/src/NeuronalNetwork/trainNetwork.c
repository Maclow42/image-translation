...
parameters *train_network(datas *data, size_t sizeSC1, size_t sizeSC2, size_t sizeSC3, float learning_rate, size_t nb_iter, int show_debug, parameters *p)
{
    ...
    matrix *X = Matrix(data->input->row, batch_size);
    matrix *y = Matrix(data->output->row, batch_size);
    ...
    for (size_t i = 0; i < nb_iter; i++)
    {
        ...
        // display 10 first columns of data->output
        if (show_debug)
        {
            printf("Output: ");
            for (size_t j = 0; j < 10; j++)
            {
                printf("%f ", data->output->data[j]);
            }
            printf("\n");
        }

        // shuffle datas
        shuffle_columns(data->input, data->output);
        ...
        for (int nb_batch = 0; nb_batch + batch_size < data->input->col; nb_batch += batch_size)
        {
            for (size_t j = 0; j < batch_size; j++)
            {
                for (size_t k = 0; k < X->row; k++)
                {
                    X->data[j * X->row + k] = data->input->data[(nb_batch + j) * X->row + k];
                }
                for (size_t k = 0; k < y->row; k++)
                {
                    y->data[j * y->row + k] = data->output->data[(nb_batch + j) * y->row + k];
                }
            }
            ...
            if (show_debug)
            {
                printf("\rProgress: %d%%", (int)(100 * (nb_batch + 1) / (float)data->input->col));
                fflush(stdout);
            }
        }
    }
